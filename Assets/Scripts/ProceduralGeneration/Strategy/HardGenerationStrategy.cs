...
{
    public class HardGenerationStrategy : IFloorGenerationStrategy
    {
            ...
            FloorGenerator generator)
        {
            ...
            while (roomsPlaced < countToGenerate && attempts < maxAttempts)
            {
                ...
                RoomConfig roomDef = RoomPlacementHelper.GetRandomRoomWithBias(roomPool, RoomDifficulty.Hard, 0.3f);
                if (roomDef is null)
                {
                    yield break;
                }
